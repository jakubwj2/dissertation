...
The frontend was designed in Unity, a cross-platform game engine. This choice created a more user-friendly experience and enhanced the GUI\@. It uses a coroutine-based network manager to communicate with the backend. An event bus publishes received responses as events, triggering UI changes and updating the current question through subscribers. Unity's multi-platform support enabled building the frontend for WebGL@, allowing it to be hosted on the same server as the API@. As a result, the frontend can serve as the landing page when users visit the server's URL, seamlessly integrating with the deployment process.

Building on this integrated approach, the project is publicly accessible at \href{https://dissertation.jakubwj.com}{dissertation.jakubwj.com}. The server is hosted on a Raspberry Pi 5. A Cloudflare tunnel provides a secure connection between the server and the public Internet. This tunnel routes traffic to an Nginx reverse proxy, which forwards requests to a Gunicorn systemctl service. Gunicorn acts as the Web Server Gateway Interface (WSGI), enabling communication between the server and the Flask backend application.
